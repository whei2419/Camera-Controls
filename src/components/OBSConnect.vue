...
<template>
  <div class="panel obs-connect-panel">
    ...
    <div v-else class="obs-form">
      ...
      <div class="field-row">
        <label>Port</label>
        <input v-model.number="port" class="field-input" type="number" placeholder="4455" :disabled="status === 'connecting'" />
      </div>
      <div class="field-row">
        <label>Password</label>
        <input v-model="password" class="field-input" type="password" placeholder="(optional)" :disabled="status === 'connecting'" />
      </div>
      <button
...
.status-dot.connecting { background: #eab308; animation: blink 0.8s infinite; }
